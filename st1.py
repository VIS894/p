...
with st.form("Input form"):
    ...
    age = st.number_input("enter your afe")
    mood = st.radio("select your mood",("happy","sad","neutral"))
    language = st.multiselect("select your language",("english","HINDI","french","germen"))
    submit = st.form_submit_button("submit")
    if submit:
        st.success(f"Name: {name}, Age : {int(age)}, Mood : {mood}, Languages: {language}")

col1 , col2, col3 =  st.columns(3)
with col1:
    st.text_input("select your name ")
    st.number_input("enter your age")
with col2:
    st.radio("enter your mood",("happy","sad","neutral"))
    st.multiselect("enter your language",("english","hindi","fremch")) 
with col3:
    st.title("output")   
...
with col2:
    colour = st.color_picker("selcet colour","#00f900")        
    st.text_area("enter your text messaage")
    st.date_input("selec a date")
    st.time_input("seleact a time")
...
#slide bar
st.sidebar.header("slide header")
st.sidebar.write("this is a slidebar text")
st.sidebar.button("click me")
option = st.sidebar.selectbox("select an option",("option1","option2","option"))

with st.container():
    st.write("this is a comtainer")
with st.expander("expand header"):
 #time.sleep(2)
 st.write("thi a exander")
with st.spinner("losdigng"):
 time.sleep(2)  
